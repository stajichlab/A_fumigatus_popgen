...
LISTCT=`ls $DIR/*.PE.bam | wc -l`
if [ $N -gt $LISTCT ]; then
 echo "$N is too big, only $LISTCT bams in $DIR"
 exit
fi

FILE=`ls $DIR/*.PE.bam | head -n $N | tail -n 1`
...
java -Xmx16g -Djava.io.tmpdir=$temp -jar $PICARD MarkDuplicates \
...
if [ ! -f $DIR/$prefix.dedup.bai ]; then
 java -jar $PICARD BuildBamIndex I=$DIR/$prefix.dedup.bam
fi
